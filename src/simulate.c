...
int branch_prediction(branch_prediction_data_t *data, int pc, int offset,
  ...
  case Bimodal:
    ...
    if (offset < 0) {                                    // taken
      ...
      if (data->BranchHistoryTable[bimodal_index] >
          0) {                                     // not stongly not taken
        data->BranchHistoryTable[bimodal_index]--; // move closer to it
      }
    }
  ...
  case gShare:
    gShare_index = ((pc >> 2) ^ data->GlobalHistoryRegister) &
                   (data->BranchHistoryTableSize - 1);
