...
int registers_read_unsigned(void *raw_data) {
  ...
  // We already confirmed that reading and writing all
  // registers works so im not gonna write
  // another 29 asserts for every register

  return 0;
}

int registers_write_unsigned(void *raw_data) {
  registers_t *registers = (registers_t *)raw_data;

  unsigned int value = (unsigned int)rand();
  int ret = write_register(registers, 0, value);
  ASSERT_EQ(ret, 0);
  ASSERT_EQ(registers->named.zero, 0);

  value = (unsigned int)rand();
  ret = write_register(registers, 1, value);
  ASSERT_EQ(ret, 0);
  ASSERT_EQ(registers->named.ra, value);
...
int simulate_sw(void *raw_data) {
  ...
  // sw a0, 0(zero)
  instruction_t sw = (instruction_t)0x00a02023;
  int ret = simulate_single(data->memory, registers, NULL, sw);

  int read_from_memory = memory_read_word(data->memory, 0);

  ASSERT_EQ(random, read_from_memory);

  return 0;
}

int simulate_bne(void *raw_data) {
  simulate_test_data_t *data = raw_data;
  registers_t *registers = &data->registers;

  // bne a5, zero, -32
  instruction_t op = (instruction_t)0xFE0790E3;

  registers->named.pc = 64;

  int ret = simulate_single(data->memory, registers, NULL, op);
  ASSERT_EQ(ret, 0);
  ASSERT_EQ(registers->named.pc, 64);

  int random = rand();
  registers->named.a5 = random;

  ret = simulate_single(data->memory, registers, NULL, op);
  ASSERT_EQ(ret, 5);
  ASSERT_EQ(registers->named.pc, 32);

  return 0;
}

int simulate_bltu(void *raw_data) {
  simulate_test_data_t *data = raw_data;
  registers_t *registers = &data->registers;

  // bltu a5, a4, 12
  instruction_t op = (instruction_t)0x00E7E663;

  registers->named.pc = 24;

  int ret = simulate_single(data->memory, registers, NULL, op);
  ASSERT_EQ(ret, 0);
  ASSERT_EQ(registers->named.pc, 24);
  ...
  ret = simulate_single(data->memory, registers, NULL, op);
  ASSERT_EQ(ret, 5);
  ASSERT_EQ(registers->named.pc, 36);

  return 0;
}

int simulate_bgeu(void *raw_data) {
  simulate_test_data_t *data = raw_data;
  registers_t *registers = &data->registers;

  // bgeu a4,a5,-56
  instruction_t op = (instruction_t)0xFCF774E3;
  ...
  random = rand();
  registers->named.a4 = random + registers->named.a5;
...
// The branch prediction for not taken. Will always not be taken. NT = Not
// taken.
...
// The branch prediction for: BTFNT = Backwards taken, Forwards not taken.
...
int branch_prediction_test_Bimodal(void *raw_data) {
  int pc = 0;
  // offset needs to be negative, for BranchHistoryTable[bimodal_index] to
  // increment
  int offset = -1;
  int size = 8;
  int bimodal_index = (pc >> 2) & (size - 1);
