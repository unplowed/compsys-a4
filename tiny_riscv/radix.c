...
void add_number(int number) {
  ...
  for (int bit_pos = MAX_BITS; bit_pos >= 1; bit_pos >>= 1) {
    ...
    // At this point there is a subtree we can use
    if (bit_pos > nptr->bit_pos) {
      ...
      if (diff) {
        ...
        *parent = nptr2;
        ...
        nptr2->number = number;
        if ((number & bit_pos) == 0) {
          // subtree is right subtree (bit we branch on is set)
          nptr2->interior.one = nptr;
          nptr2->interior.zero = NULL;
          parent = & nptr2->interior.zero;
        } else {
          // subtree is left suftree (bit we branch on is clear)
      ...
      // we may use the found (or created) node
    ...
    if (bit_pos > 1) {
      ...
      // add number at leaf. Do this simply by counting it at the proper counter
      if (number & 1)
        nptr->leaf.count_one++;
      else
...
int* take_numbers_2(struct node** parent, int* buffer, int* end) {
  if (buffer >= end) return buffer; // early out to avoid some overhead
  struct node* nptr = *parent;
  if (nptr) {
    // we have a node:
    if (nptr->bit_pos == 1) {
      // it's a leaf node. Take numbers.
      // first numbers with clear least significant bit
      while (buffer < end && nptr->leaf.count_zero) {
        *buffer++ = nptr->number & ~1;
        nptr->leaf.count_zero--;
      }
      // then numbers with set least significant bit
      while (buffer < end && nptr->leaf.count_one) {
        *buffer++ = nptr->number | 1;
        nptr->leaf.count_one--;
      }
      // if the leaf node is now "empty" it should be removed:
      if (nptr->leaf.count_zero == 0 && nptr->leaf.count_one == 0) {
        release(nptr);
        *parent = NULL;
      }
    } else {
      // it's an interior node. Take numbers from subtrees:
  ...
  while (1) {
    ...
    for (int n = 0; n < read; ++n) {
      if (numbers[n] < 0) {
        ...
        int limit = -numbers[n];
        // take requested numbers from tree and output up till 1000 numbers at a time
        while (limit > 0) {
        ...
        // a positive number is simply added to our tree
